...
def load_graph_data(dataset_name):
    ...
    """
    load_path = "/content/drive/MyDrive/CCGC_MLRC/dataset/" + dataset_name
    ...
    # ds = np.load(f'/content/citeseer_adj.npy')
    
    # ***************** Uncomment this section for Diffusion ***************
    # ds = adj
    # ds = nx.Graph(ds)
    # for i,features in enumerate(feat):
    #   ds.nodes[i]['features'] = features.tolist()
    #   ds.nodes[i]['label'] = int(label[i])
    # adj = compute_ppr(ds, 0.2)
    ...
    # ***************** Uncomment this section for Drop Edges ***************
    #for drop edges
    
    # G = nx.Graph(adj)
    # print("No edges orig: ",G.number_of_edges())
    # edges = list(G.edges())
    # num_edges_to_drop = int(0.2 * len(edges))
    # edges_to_drop = np.random.choice(len(edges), size=num_edges_to_drop, replace=False)
    # G_dropped_edges = G.copy()
    # G_dropped_edges.remove_edges_from([edges[i] for i in edges_to_drop])
    # print("No edges after: ",G_dropped_edges.number_of_edges())
    # adj_dropped_edges = nx.to_numpy_array(G_dropped_edges)
    # adj = adj_dropped_edges

    # *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    # *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*    
    # *********** Uncomment this section for Add-Edges ********
    # # G = nx.Graph(adj)
    # edges = list(G.edges())

    # # Calculate the number of edges to be added (20% of total possible edges)
    # num_edges_to_add = int(0.2 * len(edges))
    # # Get the indices of missing edges in the upper triangle of the adjacency matrix
    # missing_edge_indices = [(i, j) for i in range(G.number_of_nodes()) for j in range(i + 1, G.number_of_nodes()) if not G.has_edge(i, j)]

    # # Randomly choose 20% of the missing edges to add
    # edges_to_add_indices = np.random.choice(len(missing_edge_indices), size=num_edges_to_add, replace=False)
    # edges_to_add_indices = [missing_edge_indices[i] for i in edges_to_add_indices]
    # G_added_edges = G.copy()
    # G_added_edges.add_edges_from(edges_to_add_indices)
    # print("No edges after: ",G_added_edges.number_of_edges())

    # adj_added_edges = nx.to_numpy_array(G_added_edges)
    # adj = adj_added_edges
    # *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    # *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    
    # *********** Uncomment this section for feature_masking ***********
    #feat = randomly_mask_features(feat)
    #print("After masking: ", np.count_nonzero(feat))
...
def load_data_networks(dataset_str):
    ...
    data_path = "/content/drive/MyDrive/MLRC/CCGC_MLRC/data"
    ####### 
