...
def kmeans(
    ...
    # cuda_tensor = torch.tensor(X).to('cuda')
    # cuda_tensor = cuda_tensor.cpu().numpy()

    # unique_rows, counts = np.unique(cuda_tensor, axis=0, return_counts=True)
    # max_index = np.argmax(counts)
    # print(unique_rows[max_index],counts[max_index])
    ...
    # cuda_tensor = torch.tensor(initial_state).to('cuda')
    # cuda_tensor = cuda_tensor.cpu().numpy()
